...
from simple_knn._C import distCUDA2, distCUDA3
...
class GaussianModel:
    ...
    def reset_opacity_nonvisible(
        ...
        opacities_new = inverse_sigmoid(torch.ones_like(self.get_opacity) * 0.5)

        for filter in visibility_filters:
            opacities_new[filter] = self.get_opacity[filter]
        optimizable_tensors = self.replace_tensor_to_optimizer(opacities_new, "opacity")
        self._opacity = optimizable_tensors["opacity"]
        filter_all = torch.zeros_like(visibility_filters[0], dtype=bool)
        for filter in visibility_filters:
            filter_all = torch.logical_or(filter_all, filter)
        self.xyz_gradient_accum[~filter_all] = 0
        self.denom[~filter_all] = 0
        self.max_radii2D[~filter_all] = 0
    ...
    def reset_opacity_visible(
        ...
        opacities_new[~filter_all] = self.get_opacity[~filter_all]
        optimizable_tensors = self.replace_tensor_to_optimizer(opacities_new, "opacity")
        self._opacity = optimizable_tensors["opacity"]
