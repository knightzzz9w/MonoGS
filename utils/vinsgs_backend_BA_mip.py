...
class BackEnd(mp.Process):
    ...
    def add_next_kf2(self, frame_idx, viewpoint,  ext_points, init=False, scale=2.0, depth_map=None):
        ...
        with torch.no_grad():
            self.gaussians.compute_3D_filter(self.viewpoints.values())
    ...
    def initialize_map(self, cur_frame_idx, viewpoint):
        for mapping_iteration in range(self.init_itr_num):
            #print("gaussian size is " , self.gaussians.get_xyz.shape)
    ...
    def color_refinement(self):
        ...
        for iteration in tqdm(range(1, iteration_total + 1)):
            ...
            viewpoint_cam = self.viewpoints[viewpoint_cam_idx]
            render_pkg = render(
                viewpoint_cam, self.gaussians, self.pipeline_params, self.background, kernel_size=self.kernel_size
            )
...
#original rand + l1loss
                    ...
                    with torch.no_grad():
                        cur_render_pkg = render(  #render once
                            new_viewpoint, self.gaussians, self.pipeline_params, self.background, kernel_size=self.kernel_size
                        )
