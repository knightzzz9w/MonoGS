...
class EuRoCParser:
    def __init__(self, input_folder, start_idx=0 , end_idx=0):
        self.input_folder = input_folder
        ...
        self.color_paths = self.color_paths[start_idx:end_idx]
        self.color_paths_r = self.color_paths_r[start_idx:end_idx]
        self.n_img = len(self.color_paths)
        self.load_poses(
...
class MonocularDataset(BaseDataset):
    def __init__(self, args, path, config):
        ...
        self.iseuroc = True if config["Dataset"]["type"] == "euroc" else False
    ...
    def __getitem__(self, idx):
        ...
        image = np.array(Image.open(color_path))   #RGB
        depth = None
...
class EurocDataset(StereoDataset):
    def __init__(self, args, path, config):
        super().__init__(args, path, config)
        dataset_path = config["Dataset"]["dataset_path"]
        parser = EuRoCParser(dataset_path, start_idx=config["Dataset"]["start_idx"] , end_idx=config["Dataset"]["end_idx"])
        self.num_imgs = parser.n_img
        self.color_paths = parser.color_paths
...
class EurocMonoDataset(MonocularDataset):
    def __init__(self, args, path, config):
        super().__init__(args, path, config)
        dataset_path = config["Dataset"]["dataset_path"]
        parser = EuRoCMonoParser(dataset_path , start_idx=config["Dataset"]["start_idx"] , end_idx=config["Dataset"]["end_idx"])
        self.num_imgs = parser.n_img
        self.color_paths = parser.color_paths
        self.poses = parser.poses
...
def load_dataset(args, path, config):
    ...
    elif config["Dataset"]["type"] == "euroc" and config["Dataset"]["sensor_type"] == "stereo":
        return EurocDataset(args, path, config)
    elif config["Dataset"]["type"] == "euroc" and config["Dataset"]["sensor_type"] == "monocular":
