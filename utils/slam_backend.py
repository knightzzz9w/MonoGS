...
class BackEnd(mp.Process):
    def __init__(self, config):
        ...
        self.newframe_init_iter = config["Training"]["newframe_init_iter"]
    ...
    def initialize_map(self, cur_frame_idx, viewpoint):
        for mapping_iteration in range(self.init_itr_num):
            ...
            (
                image,
                viewspace_point_tensor,  #no use until now all zeros
                visibility_filter,   #maxradii that bigger than 0
                radii,
                depth,
                opacity,
                n_touched,
                ...
                render_pkg["render"],
                render_pkg["viewspace_points"],  # 0 
                render_pkg["visibility_filter"],  #radii > 0 
                render_pkg["radii"],
                render_pkg["depth"],
                render_pkg["opacity"],
                render_pkg["n_touched"],   #how many pixels are touched by this gaussian
            )
            ...
            with torch.no_grad():
                self.gaussians.max_radii2D[visibility_filter] = torch.max(   #history max radii
        ...
        self.occ_aware_visibility[cur_frame_idx] = (n_touched > 0).long()   #how many pixels are touched by each gaussian
        Log("Initialized map")
        return render_pkg
    ...
    def map(self, current_window, prune=False, iters=1 , map_recent = False):
        ...
        for cam_idx, viewpoint in self.viewpoints.items():   # all 
            if cam_idx in current_window_set:
                continue
            random_viewpoint_stack.append(viewpoint)  #cur view point in the windows

        if map_recent:
            iters_recent = self.newframe_init_iter
        ...
        for _ in range(iters):
            ...
            for cam_idx in range(len(current_window)):
                ...
                visibility_filter_acm.append(visibility_filter)  #radii > 0
                radii_acm.append(radii)
                n_touched_acm.append(n_touched)   # all acm 
            ...
            with torch.no_grad():
                ...
                if prune:
                    if len(current_window) == self.config["Training"]["window_size"]:
                        ...
                        for window_idx, visibility in self.occ_aware_visibility.items():  #n touched
                ...
                for idx in range(len(viewspace_point_tensor_acm)):
                    ...
                    self.gaussians.add_densification_stats(   #grad acm
        ...
        #print("mapping iter is " , iters , " and mapping time is " , map_time.toc())
        ...
        while True:
            if self.backend_queue.empty():
                ...
                if data[0] == "stop":
                    ...
                    self.add_next_kf2(cur_frame_idx, viewpoint, cur_gaussian_points , depth_map=depth_map)
