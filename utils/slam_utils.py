...
def get_loss_tracking_rgbd(
    ...
    depth_mask = depth_pixel_mask * opacity_mask   #with opacity mask
...
def get_loss_mapping(config, image, depth, viewpoint, opacity, initialization=False):
    ...
    return get_loss_mapping_rgbd(config, image_ab, depth, viewpoint , initialization = initialization)
...
def get_loss_mapping_rgb(config, image, depth, viewpoint):
    ...
    if viewpoint.trian_depthmask is not None:
...
def get_loss_mapping_rgbd(config, image, depth, viewpoint, initialization=False):
    #print("in rgbd mapping loss !")
    alpha = config["Training"]["alpha"] if "alpha" in config["Training"] else 0.95
    if initialization:
        alpha = config["Training"]["init_alpha"]
    rgb_boundary_threshold = config["Training"]["rgb_boundary_threshold"]
...
def get_loss_mapping_rgbd2(config, image, depth, viewpoint, initialization=False):
    alpha = config["Training"]["alpha"] if "alpha" in config["Training"] else 0.95
    rgb_boundary_threshold = config["Training"]["rgb_boundary_threshold"]
    image_ab = (torch.exp(viewpoint.exposure_a)) * image + viewpoint.exposure_b   #use exposure to adjust the image 
