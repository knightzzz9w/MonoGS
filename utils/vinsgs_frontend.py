...
class Vins_FrontEnd(mp.Process):
    ...
    def dataalign3(self , image_buf , campose_buf , key_point_cloud_buf): #align the data k1 k2 p1 p2 k3
        ...
        while(True):
            ...
            measurements.append([image , cur_campose , cur_key_point_cloud  , time_cur_image])
    ...
    def run(self,measurements):
        ...
        while True:
            ...
            if self.frontend_queue.empty():
                ...
                [image, cur_campose , cur_key_point_cloud , timestamp] = measurements.pop(0)
                ...
                if image_id  >= len(self.traj_gt):
                    return
                viewpoint = Camera(uid = self.cur_frame_idx, color = image, depth = None, gt_T = torch.from_numpy(self.traj_gt[image_id]["transform_matrix"]).to("cuda"), projection_matrix = self.projection_matrix
                    , fx=self.fx,
                    fy=self.fy,
                    cx=self.cx,
