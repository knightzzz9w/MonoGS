...
class SLAM_GUI:
    ...
    def rasterise(self, current_cam,kernel_size=0.1):
        if (
            self.time_shader_chbox.checked
            and self.gaussian_cur is not None
            and type(self.gaussian_cur) == GaussianPacket_mip
        ):
    ...
    def render_gui(self):
        ...
        results = self.rasterise(current_cam,kernel_size=self.pipe.kernel_size)
        if results is None:
            return
