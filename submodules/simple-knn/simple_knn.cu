...
__global__ void boxMinMax(uint32_t P, float3* points, uint32_t* indices, MinMax* boxes)
{
	...
	else
	{
		...
		me.maxx = { -FLT_MAX,-FLT_MAX,-FLT_MAX };  // for every thread
	}

	__shared__ MinMax redResult[BOX_SIZE];  // for every box
...
__global__ void boxMeanDist(uint32_t P, float3* points, uint32_t* indices, MinMax* boxes, float* dists)
{
	...
	for (int b = 0; b < (P + BOX_SIZE - 1) / BOX_SIZE; b++) // for every box
	{
		MinMax box = boxes[b];
...
__global__ void boxMeanDist2(uint32_t P1, uint32_t P2, float3* points, uint32_t* indices,  uint32_t* lookup_tables , MinMax* boxes, float* dists)
{
	int idx = cg::this_grid().thread_rank();  //0-P1-1
	if (idx >= P1)
		return;
	int indices_pos = lookup_tables[idx];
	assert(indices[indices_pos] == idx);
	float3 point = points[idx];

	float best[3] = { FLT_MAX, FLT_MAX, FLT_MAX };

	for (int i = max(0, indices_pos - 3); i <= min(P2 - 1, indices_pos + 3); i++)
	{
		if (i == indices_pos)
			continue;
		updateKBest<3>(point, points[indices[i]], best);
	...
	for (int b = 0; b < (P2 + BOX_SIZE - 1) / BOX_SIZE; b++) // for every box
	{
		MinMax box = boxes[b];
		float dist = distBoxPoint(box, point);
		if (dist > reject || dist > best[2])
...
void SimpleKNN::knn2(int P1, int P2 , float3* points, float* meanDists)
{
	float3* result;
	cudaMalloc(&result, sizeof(float3));
	size_t temp_storage_bytes;
	...
	cub::DeviceReduce::Reduce(nullptr, temp_storage_bytes, points, result, P2, CustomMin(), init);
	thrust::device_vector<char> temp_storage(temp_storage_bytes);

	cub::DeviceReduce::Reduce(temp_storage.data().get(), temp_storage_bytes, points, result, P2, CustomMin(), init);
	cudaMemcpy(&minn, result, sizeof(float3), cudaMemcpyDeviceToHost);

	cub::DeviceReduce::Reduce(temp_storage.data().get(), temp_storage_bytes, points, result, P2 , CustomMax(), init);
	cudaMemcpy(&maxx, result, sizeof(float3), cudaMemcpyDeviceToHost);

	thrust::device_vector<uint32_t> morton(P2);
	thrust::device_vector<uint32_t> morton_sorted(P2);
	coord2Morton << <(P2 + 255) / 256, 256 >> > (P2, points, minn, maxx, morton.data().get());

	thrust::device_vector<uint32_t> indices(P2);
	thrust::sequence(indices.begin(), indices.end());
	thrust::device_vector<uint32_t> indices_sorted(P2);

	cub::DeviceRadixSort::SortPairs(nullptr, temp_storage_bytes, morton.data().get(), morton_sorted.data().get(), indices.data().get(), indices_sorted.data().get(), P2);
	temp_storage.resize(temp_storage_bytes);

	cub::DeviceRadixSort::SortPairs(temp_storage.data().get(), temp_storage_bytes, morton.data().get(), morton_sorted.data().get(), indices.data().get(), indices_sorted.data().get(), P2);


	thrust::device_vector<uint32_t> lookup_table(P2);
	// thrust::for_each(thrust::counting_iterator<int>(0),
	// 				thrust::counting_iterator<int>(P2),
	// 				[indices_sorted_ptr = thrust::raw_pointer_cast(indices_sorted.data()),
	// 				lookup_table_ptr = thrust::raw_pointer_cast(lookup_table.data())] __device__ (int i) {
	// 	lookup_table_ptr[indices_sorted_ptr[i]] = i;
	// });

	// 获取指向数据的原始指针
	uint32_t* indices_sorted_ptr = indices_sorted.data().get();
	uint32_t* lookup_table_ptr = lookup_table.data().get();

	// 使用普通的for循环来替代thrust::for_each
	for (int i = 0; i < P2; ++i) {
		lookup_table_ptr[indices_sorted_ptr[i]] = i;
	}

	uint32_t num_boxes = (P2 + BOX_SIZE - 1) / BOX_SIZE;
	uint32_t num_boxes_result = (P1 + BOX_SIZE - 1) / BOX_SIZE;

	thrust::device_vector<MinMax> boxes(num_boxes);
	boxMinMax << <num_boxes, BOX_SIZE >> > (P2, points, indices_sorted.data().get(), boxes.data().get());
	boxMeanDist2 << <num_boxes_result, BOX_SIZE >> > (P1, P2,  points, indices_sorted.data().get(), lookup_table.data().get() , boxes.data().get(), meanDists);
