...
from utils.vinsgs_frontend import Vins_FrontEnd
...
class vinsgs:
    ...
    def process(self) :
        ...
        while not rospy.is_shutdown():
            with con:
                measurements = self.frontend.get_measurements3(image_buf , campose_buf , key_point_cloud_incre_buf)
...
if __name__ == '__main__':
    ...
    if config["Results"]["save_results"]:
        ...
        save_dir = os.path.join(
            config["Results"]["save_dir"], path[-3] + "_vinsgsmono_" + path[-2], current_datetime
        )
        tmp = args.config
        tmp = tmp.split(".")[0]
